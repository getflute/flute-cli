...
pub fn to_amount_number(d: rust_decimal::Decimal) -> serde_json::Value {
    ...
    serde_json::from_str::<serde_json::Value>(&d.to_string())
        .expect("Decimal::to_string() is always a valid JSON number literal")
}
...
mod tests {
    ...
    fn to_amount_number_is_a_json_number_not_a_string() {
        let v = to_amount_number(Decimal::from_str("100.00").unwrap());
    ...
    fn to_amount_number_zero_point_ten_no_float_artifact() {
        let v = to_amount_number(Decimal::from_str("0.10").unwrap());
        let s = serde_json::to_string(&v).unwrap();
